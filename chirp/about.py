...
PHASE_ONE = [
    ...
    (None, "checked models ({chat_model})", 0.18),
...
def _info_lines(version: str, notes_count: int, chat_model: str) -> list[Text]:
    ...
    lines.append(Text(""))
    lines.append(Text(f"   • {notes_count} notes · {chat_model}", style="dim"))
    credit = Text()
...
def run_about(
    ...
    notes_count = _count_notes(settings.directories.notes)
    ...
    with Live(Group(*lines), console=console, refresh_per_second=20) as live:
        ...
        for spinner_msg, done_template, dwell in PHASE_ONE:
            done_msg = done_template.format(
                notes_count=notes_count, chat_model=chat_model
            )
        ...
        # Phase 2 — clear the region and paint the logo line-by-line
        lines[:] = [_prompt_line(), Text("")]
        live.update(Group(*lines))
        logo_start = len(lines)
        for idx in range(len(LOGO_ROWS)):
            lines.append(_logo_line(idx, beak_open=False))
            live.update(Group(*lines))
            sleep(LOGO_PAINT_INTERVAL * scale)

        beak_row = next(i for i, row in enumerate(LOGO_ROWS) if row.has_beak)
        beak_idx = logo_start + beak_row

        # Phase 3 — info lines type in while the beak keeps chirping
        info = _info_lines(version, notes_count, chat_model)
        open_state = False
