...
## Found during: regression corpus capture 6.1 (2026-06-08)

- **Ollama note-gen corrupts long transcripts (missing `num_ctx`).** `NoteGenerator._call_ollama` sends the entire transcript in one prompt and sets only `num_predict` (output budget), never `num_ctx` (input window). Ollama then silently caps input at its default 4096-token context, so any transcript over ~4k tokens loses the format instructions and the model echoes input-shaped `<transcript>` XML; the XML parser falls back to "Unable to parse structured notes from AI response". Two failure modes by overflow severity: heavy overflow (5,906 and 6,153 words) → XML-tag bleed; mild overflow (4,097 words, just past the window) → structurally-valid but **empty** notes ("No summary available", all sections "None"). **Confirmed config-only:** re-running the identical model + prompt with `options.num_ctx=16384` parses cleanly. This is a real defect users hit today on long recordings, not specific to the migration. All three corpus long entries are degraded baselines committed as the genuine "before" per the story owner's keep-as-is decision: `karpathy-interview-long` + `agentic-coding-talk-long` (XML bleed) and `parliament-recession-debate-long` (empty). Story 6.6's blinded scoring should be aware all three are degraded. Fix is a one-liner (set `num_ctx` from a sized budget, or chunk the transcript). [notes/note_generator.py:359]

## Deferred from: code review of 7.5-docs-sweep (2026-06-12)

- **`models.llm` is a vestigial Ollama-era field** carrying the default `"llama3.1:8b"`. Post-migration the actual chat model is the registry's `default_chat` alias (resolved at call time as `model="default"`); `models.llm` only feeds display/metadata — `chirp config --list`, the about screen, and `meta.toml`'s `llm_model`. The Ollama-style default is misleading. The right fix surfaces the **resolved registry alias** in those display sites (not a flip of the default to the literal `"default"`, which is itself uninformative in note metadata). Touches `config/settings.py`, `chirp/cli.py` (config command + `--llm-model` help "e.g. llama3.1:8b"), `chirp/about.py`, `notes/note_generator.py`/`batch_processor.py` meta writes, and their tests — beyond 7.5's docs-sweep scope. Flagged by Copilot on PR #96. [config/settings.py:ModelsConfig.llm]
