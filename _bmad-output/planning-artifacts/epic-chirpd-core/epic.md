...
| 3.8 | Enable embed-model loading + pooled inference in `MLXBackend` (remediation of an escaped 3.6 defect; blocks cutover 6.3) | FR2, FR22 (embed role, completed) | 3.6 | [stories/3.8-embed-model-load-and-pooled-inference.md](stories/3.8-embed-model-load-and-pooled-inference.md) |

> **Post-epic remediation (3.8).** This epic was marked Done after 3.7, but manual smoke of EPIC-MODEL-REGISTRY story 4.4 surfaced an escaped defect in 3.6: `MLXBackend.load` routes every role through `mlx_lm.load`, so embed (`bert`/`bge`) models cannot load, and the embed inference path returns token-level rather than pooled vectors. Story 3.8 remediates both. It is sequenced after 3.7 and is a prerequisite for EPIC-INTEGRATION-CUTOVER story 6.3, which is where embed gets its first real consumer and end-to-end verification. See [`bug-chirpd-registry-reread-and-embed-load.md`](../../implementation-artifacts/bug-chirpd-registry-reread-and-embed-load.md) Issue 1. (Issue 2 from the same report — the per-op registry re-read — was fixed directly in `chirpd/state.py`; not story-tracked.)
